    ...
    try {
        ...
        try {
            ...
            if (user.isTokenEmpty('verifyToken')) {
                res.json(stat('alreadyVerified', ''));
            } else {
                // If not verified, empty the user's verifyToken property
                user.removeToken('verifyToken')
                    .then(() => res.json(stat('verified', '')))
                    .catch(e => console.log('Error in /verify'));
            }
        } catch (e) {
            res.status(400).json(stat('failed', 'Sorry, something went wrong. Double check everything.'));
        }
    } catch (e) {
        ...
        if (e.name === 'JsonWebTokenError') {
            // Send error message
            res.status(400).json(stat('failed', 'Invalid Token!'));
        }
    }
};
