 ...
 * and the refresh tokens expires within 90 days
 */
...
const authenticate = async (req, res, next) => {
    ...
    if (authToken === null || authToken === undefined)
        return res.status(401).json(stat('failed', 'Sorry, authentication is required.'));
    ...
    try {
        ...
        if (e.name === 'TokenExpiredError') {
            res.status(401).json(stat('expired', 'Sorry, authentication is required.'));
        } else {
            res.status(401).json(stat('failed', 'Sorry, authentication is required.'));
        }
    }
};
